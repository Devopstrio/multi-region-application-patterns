...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    { name: 'Resilience Hub', path: '/', icon: LayoutDashboard },
    { name: 'Regional Health', path: '/health', icon: Signal },
    { name: 'Traffic Steering', path: '/routing', icon: Zap },
    { name: 'Geo-Replication', path: '/replication', icon: ArrowRightLeft },
    { name: 'Failover Control', path: '/failover', icon: RotateCcw },
    { name: 'Resilience Policies', path: '/policies', icon: ShieldCheck },
    { name: 'Global Gateway', path: '/gateway', icon: Globe },
    { name: 'Cluster Topology', path: '/topology', icon: Server },
    { name: 'Observability', path: '/monitoring', icon: Activity },
    { name: 'SLA Compliance', path: '/sla', icon: FileText },
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          ...
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-emerald-400 bg-clip-text text-transparent">Geo.Ops</span>}
        </div>
          ...
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">Resilience Architect</p>
                <p className="text-xs text-slate-500 truncate">Multi-Region SRE</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search regional services, failover history, replication events..." 
